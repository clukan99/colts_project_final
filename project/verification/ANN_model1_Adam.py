...
X_test = full_X_train_ver[(full_X_train_ver['event_name'] == 'CLT22LAC')]
X_train = full_X_train_ver[(full_X_train_ver['event_name'] != 'CLT22LAC')]

y_test = y_test[(y_test['event_name'] == 'CLT22LAC')]
y_train = y_train[(y_train['event_name'] != 'CLT22LAC')]
#################################################################################################################################

########################################Proper formatting must be completed prior to model initiation ###########################

test_UniqueID = X_test[['UniqueID']]
test_UniqueID['y_test'] = y_test['isAttended']

X_train = X_train.drop(labels = ['event_name', 'UniqueID'], axis= 1)
X_test = X_test.drop(labels = ['event_name', 'UniqueID'], axis = 1)

y_train = y_train[['isAttended']]
y_test = y_test[['isAttended']]
...
output_size = 1
hidden_layer1_size = [input_size//2,input_size//4, input_size//6]
n_iters = [30,60]
    ...
    for hidden_layer1_size2 in hidden_layer1_size:
        for dropout2 in dropout:
            for learning_rate2 in learning_rate:
                ...
                scores.to_csv('../../verification_data/ANN1_accuracy_scores_PCA.csv')
                ########
                now = datetime.now()
